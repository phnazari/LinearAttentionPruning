...
class HighRankGatedLinearAttention(nn.Module):
    r"""
    ...
    def __init__(
        ...
        self.k_proj = nn.Linear(hidden_size, self.key_dim_per_group * self.rank, bias=False)
        self.v_proj = nn.Linear(hidden_size, self.value_dim_per_group * self.rank, bias=False)
        ...
        **kwargs: Unpack[dict],
        ...
        if self.use_short_conv:
            ...
            k, conv_state_k = self.k_conv1d(
                x=rearrange(self.k_proj(hidden_states), "b t (r h) -> b (t r) h", r=self.rank),
                cache=conv_state_k,
                output_final_state=use_cache,
                cu_seqlens=cu_seqlens,
            )
            v, conv_state_v = self.v_conv1d(
                x=rearrange(self.v_proj(hidden_states), "b t (r h) -> b (t r) h", r=self.rank),
                cache=conv_state_v,
                output_final_state=use_cache,
                cu_seqlens=cu_seqlens,
            )
