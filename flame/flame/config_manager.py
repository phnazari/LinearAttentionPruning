...
class JobConfig:
    ...
    def __init__(self):
        ...
        # Parameter freezing configs (for efficient finetuning)
        self.parser.add_argument(
            "--model.freeze_non_target",
            action="store_true",
            help="Freeze all parameters except target modules (efficient finetuning without LoRA)",
        )
        self.parser.add_argument(
            ...
            help="Comma-separated list of module names to keep trainable when freezing. Default: q_proj,k_proj (compressed weights)",
        )
        
        # LoRA configs (for parameter-efficient finetuning)
        self.parser.add_argument(
            "--model.use_lora",
            action="store_true",
            help="Apply LoRA (Low-Rank Adaptation) to target modules using PEFT",
        )
        self.parser.add_argument(
            "--model.lora_target_modules",
            type=string_list,
            nargs="+",
            default=["q_proj", "k_proj", "v_proj", "o_proj", "b_proj", "g_proj", "gate_proj", "up_proj", "down_proj", "lm_head"],
            help="Comma-separated list of module names to apply LoRA. Default: all attention, MLP, and output layers",
        )
        self.parser.add_argument(
            "--model.teacher_model_path",
            type=str,
            default=None,
            help="Path to the teacher model checkpoint for knowledge distillation",
        )
        self.parser.add_argument(
            "--model.teacher_model_config",
            type=str,
            default=None,
            help="Path to the teacher model config (optional)",
        )
        self.parser.add_argument(
            "--training.distillation_loss_weight",
            type=float,
            default=0.0,
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=8,
            help="LoRA rank (r parameter). Higher rank = more parameters but better capacity. Default: 8",
        )
        self.parser.add_argument(
            "--model.lora_alpha",
            type=int,
            default=16,
            help="LoRA alpha scaling factor. Often set to 2*rank. Default: 16",
        )
        self.parser.add_argument(
            "--model.lora_dropout",
            type=float,
        ...
        self.parser.add_argument(
            ...
            help="LoRA bias training mode. Options: none, all, lora_only. Default: none",
        )
